            ...
            <nav className="bg-white border-b border-gray-100">
                    ...
                    <div className="flex justify-between h-16">
                        ...
                        <div className="hidden sm:flex sm:items-center sm:ms-6">
                            ...
                            <Dropdown>
                                ...
                                <DropdownMenu>
                                    <DropdownItem href={route('profile.edit')}>Edit Profile</DropdownItem>
                                    <DropdownItem href={route('logout')} method="post">Logout</DropdownItem>
                                </DropdownMenu>
                            </Dropdown>
                            </div>
                        </div>
                ...
                <div className={(showingNavigationDropdown ? 'block' : 'hidden') + ' sm:hidden'}>
                    <div className="pt-2 pb-3 space-y-1">
                        ...
                        <ResponsiveNavLink href={route('create.post')} active={route().current('create.post')}>
                            <Text>Create Post</Text>
                        </ResponsiveNavLink>
                        <ResponsiveNavLink href={route('notifications')} active={route().current('notifications')}>
                            <Text>Notifications</Text>
                        </ResponsiveNavLink>
                        <ResponsiveNavLink href={`/profile/${user.id}`} active={currPathName === `/profile/${user.id}`}>
                            <Text>Profile</Text>
                        </ResponsiveNavLink>
                    </div>
                    ...
                    <div className="pt-4 pb-1 border-t border-gray-200">
                        <div className="px-4">
                            <Strong className="font-medium text-base text-gray-800">{user.name}</Strong>
                            <Text className="font-medium text-sm text-gray-500">@{user.username}</Text>
                        </div>
