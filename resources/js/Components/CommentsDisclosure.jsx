...
function CommentsDisclosure({ post: initialPost , isOpen, auth}) {
    ...
    <Disclosure>
      {({ open }) => (
        <>
          ...
          >
            <Disclosure.Panel className="pt-4 pb-2text-sm ">
              {post.comments.map((comment) => (
                <div key={comment.id} className="flex mb-2 items-center space-x-2">
                  ...
                  <div className="flex-1">
                    <span><strong className='mr-2'><Link href={`/profile/${comment.user.id}`}>{comment.user.username}</Link></strong>{comment.content}</span>
                    <div className='flex'>
                        <div className='text-xs mr-2'>{formatDateTime(comment.updated_at)} ago</div>
                        {comment.user.id === auth.user.id && (
                            <>
                                <button className="text-gray-500 hover:text-gray-900 focus:outline-none focus-visible:ring focus-visible:ring-gray-500 focus-visible:ring-opacity-75">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
                                    <path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                </svg>
                                </button>
                                <button onClick={(e) => deleteComment(e, comment)} className="text-gray-500 hover:text-red-600 focus:outline-none focus-visible:ring focus-visible:ring-gray-500 focus-visible:ring-opacity-75">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
